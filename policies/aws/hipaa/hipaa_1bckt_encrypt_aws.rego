...
violations[msg] if {
	...
	msg := sprintf("HIPAA Violation: S3 bucket '%v' is missing a 
					dedicated encryption configuration resource.", [bucket_name])
}
...
violations[msg] if {
	...
	msg := sprintf("HIPAA Violation: S3 encryption configuration '%v' 
					does not use 'aws:kms'. Standard S3 encryption (AES256) is forbidden. 
					Customer-Managed Keys (CMK) must be used.", [resource.address])
}
...
violations[msg] if {
	...
	msg := sprintf("HIPAA Violation: S3 encryption configuration '%v' 
	is using the default AWS-managed key ('aws/s3'). 
	A Customer-Managed Key (CMK) ARN must be specified.", 
	[resource.address])
}
